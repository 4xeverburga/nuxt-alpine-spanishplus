...
<template>
  <header :class="alpine.header.position || 'left'">
    <div class="menu">
      <button :aria-label="$t('nav.menu')" @click="(show = !show)">
    ...
    <div class="logo">
      <NuxtLink v-if="alpine.header.logo" :to="alpine.header.logo.href || '/'">
      ...
      <NuxtLink v-else :to="alpine.header.logo?.href || '/'" class="fallback">
        {{ alpine.title }}
      </NuxtLink>
    </div>
